...
$test_scenarios = $pow->run_http_api(sprintf(Constants::$pow_fetch_scenarios, $plid, $pn, $v));

for ($i=0; $i < count($test_scenarios); $i++) {
	$tsid = $test_scenarios[$i][0];
	$details = $pow->run_http_api(sprintf(Constants::$pow_fetch_details_by_pn_version_tsid, $plid, $pn, $v, $tsid));
	array_push($test_scenarios[$i], $details);
}
